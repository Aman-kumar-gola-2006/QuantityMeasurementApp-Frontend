...
export class AuthService {
  ...
  googleLogin(idToken: string): Observable<string> {
    return this.http.post(`${this.api}/auth/google`, { idToken }, { responseType: 'text' }).pipe(
      tap(token => {
        localStorage.setItem('token', token);
        // Note: localStorage userEmail will be set after decoding if needed, 
        // or the backend could return a more complex object.
      })
    );
  }
